...
public class ArrivedCommandTest {

    private OrderDAO orderDAO = new OrderDAO();
    private CityDAO cityDAO = new CityDAO();
    private UserDAO userDAO = new UserDAO();

    private ArrivedCommand arrivedCommand = new ArrivedCommand();
    ...
    public void shouldChangeStatusToArrived() throws IOException, ServletException {
        ...
        arrivedCommand.execute(request, response);

        Delivery founded = orderDAO.findDeliveryById(delivery.getId());

        assertEquals(DeliveryStatus.ARRIVED, founded.getLastStatus());
